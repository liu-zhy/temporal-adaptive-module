# Code for Online TAM Testing on Kinetics-400 Dataset
...
from ops.online_kinetics400_dataset import TSNDataSet
from ops.models import TSN
from ops.transforms import *
from ops import online_kinetics400_dataset_config
...
## dense-N denotes densely sample N clips, uniform-N denotes uniformly sample 10 clips.
...
parser.add_argument('--num_set_segments',
                    type=int,
                    default=1,
...
def parse_shift_option_from_log_name(log_name):
    log_name = log_name.split('/')[-2]
    print(log_name)
    has_tam = 'tam' in log_name
    if 'RGB' in this_weights:
        modality = 'RGB'
    else:
        modality = 'Flow'
    backbone = this_weights.split('/')[-2].split('_')[2]
    return has_tam, modality, backbone
...
for this_weights, this_test_segments, test_file in zip(weights_list,
    ...
    num_class, args.train_list, val_list = online_kinetics400_dataset_config.return_dataset(
    ...
    data_loader = torch.utils.data.DataLoader(
        TSNDataSet(
            ...
            transform=torchvision.transforms.Compose([
                cropping,
                Stack(roll=(backbone in ['BNInception', 'InceptionV3'])),
                ToTorchFormatTensor(
                    div=(backbone not in ['BNInception', 'InceptionV3'])),
                GroupNormalize(net.input_mean, net.input_std),
            ]),
            test_sample=args.sample
            # dense_sample=True if args.dense_sample == 'local' else False,
        ),
...
def eval_video(video_data, net, this_test_segments, modality):
    ...
    with torch.no_grad():
        ...
        # clips = 1
        # if args.dense_sample != 'no':
        #     clips = 10  # 10 clips for testing when using dense sample
        clips = int(args.sample.split("-")[-1])
        if modality == 'RGB':
            length = 3
        elif modality == 'Flow':
            length = 10
        ...
        if has_tam:
            data_in = data_in.view(batch_size * num_crop * clips,
                                   this_test_segments, length, data_in.size(2),
                                   data_in.size(3)) # this_test_segments相当于n_segment，是时序
            # clips是test_indices时，start_idx会变，dense的话就是间隔一定，但起始会不一样
            # clips就相当于是多组的输入的感觉，帧之间间隔一致（dense的话）但起始不同，帧就会有不一样
            # num_crop是对一张256的图三个固定区域crop出几张224x224
        rst = net(data_in)
        rst = rst.reshape(batch_size, num_crop * clips, -1).mean(1)
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        this_label = None
        for n_seg, (_, (data,
                        label)), net, modality in zip(test_segments_list,
                                                      data_label_pairs,
                                                      net_list, modality_list):
        ...
        prec1, prec5 = accuracy(torch.from_numpy(ensembled_predict),
                                this_label,
                                topk=(1, 5))
        ...
        if i % 20 == 0:
            print('video {} done, total {}/{}, average {:.3f} sec/video, '
                  'moving Prec@1 {:.3f} Prec@5 {:.3f}'.format(
...
if args.csv_file is not None:
    ...
    with open(test_file_list[0].replace('test_videofolder.txt',
                                        'category.txt')) as f:
