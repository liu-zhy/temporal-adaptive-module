...
for this_weights, this_test_segments, test_file in zip(weights_list,
    ...
    for k, v in sf_weights.items():
        ...
        if 'self_conv.conv_g' in k:
            k = k.replace('self_conv.conv_g', 'tam.L')
        # print(k)
        if tam_ckpt['.'.join(k.split('.')[1:])].size() != v.size():
            print(f"{tam_ckpt['.'.join(k.split('.')[1:])].size()} : {v.size()}")
        # base_dict[k] = v
        tam_ckpt['.'.join(k.split('.')[1:])] = v
        if '.'.join(k.split('.')[1:]) not in tam_ckpt.keys():
            print(k)
    # sf_ckpt['state_dict'] = base_dict
    sf_ckpt['state_dict'] = tam_ckpt
    file_name = './checkpoint/kinetics_RGB_resnet50_tam_avg_segment16_e100_dense'
    os.makedirs(file_name, exist_ok=True)
    torch.save(sf_ckpt, f'{file_name}/ckpt.best.pth.tar')
    base_dict = {
        '.'.join(k.split('.')[1:]): v
        for k, v in list(base_dict.items())
    }
    print(len(sf_weights.keys()))
    print(len(tam_ckpt.keys()))
    # exit()
    net.load_state_dict(tam_ckpt)
